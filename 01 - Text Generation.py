...
class Genetic:
    def __init__(self, population_number,targeted_text):
        ...
        self.List_of_letters = "abcçdefgğhıijklmnoöprşstuüvyzABCÇDEFGĞHIİJKLMNOÖPRŞSTUÜVYZ "
    ...
    def Kill_weaks(self):
        for member in self.Population:
            if member.Score <= self.Weak_Fitness:
    ...
    def main(self):
        ...
        print(f"Your target = {self.Targeted_Text}, Ulaşılan = {self.Winner}"
              f"\nGerçekleşen Jenarasyon sayısı = {self.Generation_Timer}")

Program = Genetic(400,"Mustafa Kemal Atatürk")
Program.main()
